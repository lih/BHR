...
-- | Create a new KademliaInstance corresponding to a given Id on a given port, with a given integrity check
create :: (Serialize i, Ord i, Serialize a, Eq a, Eq i) =>
    Int -> i -> (i -> a -> Bool) -> IO (KademliaInstance i a)
create port id integ = do
    rq <- emptyReplyQueue
    h <- openOn (show port) id rq
    inst <- newInstance id integ h
    start inst rq
    return inst

...
newInstance :: (Serialize i) =>
               i -> (i -> a -> Bool) -> KademliaHandle i a -> IO (KademliaInstance i a)
newInstance id integ handle = do
    tree <- atomically . newTVar . T.create $ id
    values <- atomically . newTVar $ M.empty
    threads <- atomically . newTVar $ M.empty
    return . KI handle (KS tree values) integ $ threads
...
timeoutNode (KI _ (KS sTree _) _ _) id = atomically $ do
    tree <- readTVar sTree
...
lookupNode (KI _ (KS sTree _) _ _) id = atomically $ do
    tree <- readTVar sTree
...
insertValue key value (KI _ (KS _ values) integ _) = atomically $ when (integ key value) $ do
...
pingProcess :: (Serialize i, Serialize a, Eq i) => KademliaInstance i a
            -> Chan (Reply i a) -> IO ()
pingProcess (KI h (KS sTree _) _ _) chan = forever $ do
...
spreadValueProcess :: (Serialize i, Serialize a, Eq i) => KademliaInstance i a
                   -> IO ()
spreadValueProcess (KI h (KS sTree sValues) _ _) = forever $ do
    threadDelay hour
...
expirationProcess inst@(KI _ _ _ valueTs) key = do

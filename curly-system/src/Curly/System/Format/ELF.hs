...
class (Format (Elf16 sys),Format (Elf32 sys),Format (Elf64 sys),Format (ElfN sys)) => ElfSystem sys where
  data Elf16 sys :: *
  data Elf32 sys :: *
  data Elf64 sys :: *
  data ElfN sys :: *
...
instance (ElfEnum flags u,HasFlagOps u,Serializable u) => Serializable (ElfFlags flags) where
  encode (ElfFlags fs) = encode (foldr addFlags nullFlag (map fromElfEnum fs))
instance (ElfEnum flags u,Ord flags,HasFlagOps u,Format u) => Format (ElfFlags flags) where
...
enumEncode :: (ElfEnum t u, Serializable u) => t -> Builder
enumEncode x = encode (fromElfEnum x)
enumDatum :: (ElfEnum t u, Format u) => Parser Bytes t
...
instance Serializable ElfABI where encode = enumEncode
instance Format ElfABI       where datum = enumDatum
...
instance ElfEnum ElfBits Word8 where
  fromElfEnum EB_32 = 1
  fromElfEnum EB_64 = 2
instance Serializable ElfBits where encode = enumEncode
instance Format ElfBits       where datum = enumDatum
...
instance Serializable ElfOrder where encode = enumEncode
instance Format ElfOrder       where datum = enumDatum
...
instance ElfSystem sys => Serializable (ElfType sys) where encode = enumEncode
instance ElfSystem sys => Format (ElfType sys) where datum = enumDatum
...
instance ElfSystem sys => Serializable (ElfMachine sys) where encode = enumEncode
instance ElfSystem sys => Format (ElfMachine sys) where datum = enumDatum
...
instance ElfSystem sys => Serializable (ElfPHType sys) where encode = enumEncode
instance ElfSystem sys => Format (ElfPHType sys) where datum = enumDatum
...
instance ElfSystem sys => Serializable (ElfSHType sys) where encode = enumEncode
instance ElfSystem sys => Format (ElfSHType sys) where datum = enumDatum
...
data ElfSymInfo = ElfSymInfo {
  ...
  esyi_bind :: ElfSymBind
                   ...
                   deriving Show
instance Serializable ElfSymInfo where
  encode (ElfSymInfo t b) = encode (fromElfEnum t .|. (fromElfEnum b `shiftL` 4))
instance Format ElfSymInfo where
...
instance Serializable ElfSymVisibility where encode = enumEncode
instance Format ElfSymVisibility       where datum = enumDatum
...
instance ElfSystem sys => Serializable (ElfRelInfo sys) where
  encode (ElfRelInfo s t) = case bits of
    EB_32 -> encode (e_32 $ (e_from32 s`shiftL`8) .&. fromIntegral (fromElfEnum t) :: Elf32 sys)
    EB_64 -> encode (e_64 $ (fromIntegral (e_from32 s)`shiftL`32) .&. fromIntegral (fromElfEnum t) :: Elf64 sys)
    where (_,_,bits,_) = e_abi (undefined :: sys)
...
instance ElfSystem sys => Serializable (ElfRela sys)
instance ElfSystem sys => Format (ElfRela sys)

instance forall sys. ElfSystem sys => Serializable (Elf sys) where
  encode (Elf t entry progs) = ehdr + fold [by bytesBuilder dat
    ...
    where vers :: Num n => n ; vers = 1
          enc16 n = encode (e_16 n :: Elf16 sys)
          enc32 n = encode (e_32 n :: Elf32 sys)
          encN n  = encode (e_n n :: ElfN sys)
          sys = sys :: sys
          (m,a,b,o) = e_abi sys
          ...
          pHdrs = map pHdr hdrOffs
            where pHdr (off,ElfSection { es_contents = ElfProgbits vaddr (r,w,x) dat }) = encode ElfProgramHeader {
                    eph_type = PHT_Load,
          ...
          sHdrs = map encode (nullSectionHeader : strTHdr : map sHdr hdrOffs :: [ElfSectionHeader sys])
          ...
          symtabs = map stHdr hdrOffs
            where stHdr (_,ElfSection _ (ElfSymTab syms)) = fold [sym n s v | (n,(s,v)) <- syms^.ascList]
                  ...
                  sym n s v = let shndx = head [i | (s',i) <- zip (map es_name progs) [2..], s==s']
                              in encode nullSym { esy_visibility = ESV_Protected,
...
sHN_ABS :: Num n => n
sHN_ABS = 65521
instance forall sys. (ElfShow sys, ElfSystem sys) => Format (Elf sys) where
  datum = do
    file <- remaining
    elfH <- datum
